...
# データ処理
def kmeans_main(df_plsa_input):

    # PLSA後データ
    df_plsa_4 = pd.read_csv(filename_plsa_4, index_col = 0).fillna(0)
    df_plsa_6 = pd.read_csv(filename_plsa_6, index_col = 0).fillna(0)
    df_plsa_8 = pd.read_csv(filename_plsa_8, index_col = 0).fillna(0)
    df_plsa_10 = pd.read_csv(filename_plsa_10, index_col = 0).fillna(0)

    print(df_plsa_10.head())
    sys.exit()
    ...
    # kmeans
    kmeans_4 = create_kmeans(df_plsa_4, 'plsa_4')
    kmeans_6 = create_kmeans(df_plsa_6, 'plsa_6')
    kmeans_8 = create_kmeans(df_plsa_8, 'plsa_8')
    kmeans_10 = create_kmeans(df_plsa_10, 'plsa_10')
    kmeans_o = create_kmeans(df_table, 'plsa_o')
...
#    os.mkdir("/home/gixo/plsa/output_kmeans/" + starttime)
    df_origin.to_csv(OUTPUT_PLSA + starttime + fn + '_out_plsa_kmeans{}.csv'.format(N))
...
def plsa_main():
    ...
    df = data.set_index(index).unstack().reset_index().rename(columns={'level_0':column, 0:value})
    ...
    Pz_x_df.to_csv(OUTPUT_PLSA + '/{}_out_{}.csv'.format(starttime, x.name))
    ...
    Pz_y_df.to_csv(OUTPUT_PLSA + '/{}_out_{}.csv'.format(starttime, y.name))
