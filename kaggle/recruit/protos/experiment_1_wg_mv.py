...
def main():
    ...
    base_cols = list(b_mark.columns)
    fe_merge = air_vi[['air_store_id', 'visit_date']].sort_values(by=['air_store_id', 'visit_date']).reset_index(drop=True)

    valid_list = []
    feature_list = []
    feature_path = glob.glob('../feature/*.csv')
    for path in feature_path:
        feature = pd.read_csv(path)
        feature_list.append(feature.columns.values[0])

        d_feature = pd.concat([fe_merge, feature], axis=1)
        b_mark = b_mark.merge(d_feature, on=['air_store_id', 'visit_date'], how='left')

    b_mark.fillna(0, inplace=True)

    for i in range(1, len(feature_list), 1):
        tmp_tuple = combinations(feature_list, i)
        for elem in tmp_tuple:
            valid_list.append(base_cols+list(elem))

    incremental_train(b_mark, target, categorical_feature, valid_list, 1)
