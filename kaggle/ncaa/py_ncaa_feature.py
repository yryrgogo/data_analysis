...
output_path = '../input/' + start_time + '_ncaa_feature_set.csv'
...
# feature_setting
season_list = np.arange(1992, 2019, 1)
#  season_list = np.arange(2004, 2019, 1)
    ...
    for h, location in enumerate(['all', 'away', 'home', 'neutral']):
        ...
        if h == 0:
            ...
            this_season_f_set = this_season_f_set.merge(
                ...
                this_season_f, on=['teamid', 'season', 'match_no'], how='left')
...
prime = df[['teamid', 'coachname', 'firstd1season', 'season', 'daynum', 'match_no',
            'teamid_2', 'match_no_2', 'result', 'score']].drop_duplicates()
...
fe_result.drop(['match_no', 'match_no_2', 'result_2'], axis=1, inplace=True)
...
#  fe_result['location'] = fe_result['location'].map(lambda x: 2 if x == 'H' else 1 if x == 'N' else 0)

print('feature set downloading...')
fe_result.to_csv(output_path, index=False)
print('feature set download end')
